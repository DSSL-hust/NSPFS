...
void device_init(void)
{
	...
	for (i = 1; i < g_n_devices + 1; i++) {
		if(i==g_root_dev){
			...
			g_bdev[i]->map_base_addr = g_bdev[i]->storage_engine->init(i, g_dev_path[i]);
	...
	bcache_rwlock = (pthread_rwlock_t *)userfs_zalloc(sizeof(pthread_rwlock_t));
...
void device_commit(void *arg){
	int i= (int)arg;
	
	g_bdev[i]->storage_engine->commit(i);
...
void device_exit(void){
	int i;
	for (i = 1; i < g_n_devices + 1; i++) {
		g_bdev[i]->storage_engine->exit(i);
		bdev_free(g_bdev[i]);
	}
}
...
struct block_device *bdev_alloc_fast(uint8_t dev_id, int blocksize_bits)
{
	...
	bdev = (struct block_device *)userfs_alloc(sizeof(struct block_device));
		...
		addr_t block_nr, uint8_t mode)
{
	...
	bh = (struct buffer_head *)userfs_alloc(sizeof(struct buffer_head));
	if (!bh)
		return NULL;
	bh->b_bdev = bdev;
	bh->b_state = 0;
...
struct buffer_head *get_bh_from_cache(block_key_t key, uint8_t mode)
{
	...
	if (bh) {
		...
		if (bh->b_size != size) {
			...
			/* userfs_assert(bh->b_blocknr == blockno); */
			__sync_fetch_and_add(&bh->b_count, 1);
			return bh;
		/* } */
	}
...
int bh_submit_read_sync_IO(struct buffer_head *bh)
{
	...
	int ret;
	struct storage_operations *storage_engine = g_bdev[bh->b_dev]->storage_engine;

	if (bh->b_offset) {
		ret = storage_engine->read_unaligned(bh->b_dev, bh->b_data, 
				bh->b_blocknr, bh->b_offset, bh->b_size);
	} else {
		ret = storage_engine->read(bh->b_dev, bh->b_data, bh->b_blocknr, bh->b_size);
	}
	if (ret != bh->b_size){
...
#if 0
struct buffer_head *userfs_read_sync_IO(uint8_t dev, addr_t block_nr,
		uint32_t size, int *ret)
{
	...
	//userfs_assert(blockno <= disk_sb[dev].size);
	...
	/* if (b->b_size != size) { */
	/* 	userfs_debug("block size %u, requested %u\n", b->b_size, size); */
...
int userfs_write_nvm(uint8_t *buf,uint64_t offset, size_t size){
	memcpy(writestart+offset, buf, size);
	return 0;
}

int userfs_write_ssd(uint8_t devid,uint8_t *buf,uint64_t offset, size_t size){
	int ret;
	int fd;
	switch (devid){
		case 1:
...
int userfs_direct_write(int fd, uint8_t *buf,uint64_t offset, size_t size){
	int ret;
...
int userfs_write(struct buffer_head *b)
{
	int ret;
	struct storage_operations *storage_engine;
	userfs_assert(b->b_size > 0);
...
int userfs_io_wait(uint8_t dev, int read)
{
#ifndef USE_SSD
	if (dev == g_ssd_dev)
		return 0;
...
int userfs_readahead(uint8_t dev, addr_t blockno, uint32_t io_size)
{
#ifndef USE_SSD
	if (dev == g_ssd_dev)
		return 0;
		       ...
		       void *buf)
{
	...
	userfs_assert(blk_size == g_block_size_bytes);
	userfs_assert(count == 1);
		...
		addr_t block_nr, int page_size)
{
	int ret;
	struct buffer_head *bh;
	bh = (struct buffer_head *)userfs_alloc(
...
void wait_on_buffer(struct buffer_head *bh, int isread)
{
	...
	userfs_io_wait(bh->b_dev, isread);
}
...
static void try_to_sync_buffers(struct block_device *bdev)
{
	...
	if (bdev->b_devid == g_ssd_dev)
		userfs_io_wait(g_ssd_dev, 0);

	userfs_debug("%lu blocks are synced\n", i);
}
...
void sync_all_buffers(struct block_device *bdev)
{
	...
	while (1) {
		...
		if (cur) {
			sync_dirty_buffer(cur);
			userfs_debug("[dev %d], block %lu synced\n", 
