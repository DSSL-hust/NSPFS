//
// File-system system calls.
// Mostly argument checking, since we don't trust
// user code, and calls into file.c and fs.c.
//

#include <stdlib.h>
#include <stdio.h>
#include <fcntl.h>

#include "global/global.h"
#include "global/types.h"
#include "filesystem/stat.h"
#include "filesystem/fs.h"
#include "filesystem/file.h"

#ifdef __cplusplus
extern "C" {
#endif

#define _min(a, b) ({\
		__typeof__(a) _a = a;\
		__typeof__(b) _b = b;\
		_a < _b ? _a : _b; })

/* note for this posix handlers' (syscall handler) return value and errno.
 * glibc checks syscall return value in INLINE_SYSCALL macro.
 * if the return value is negative, it sets the value as errno 
 * (after turnning to positive one) and returns -1 to application.
 * Therefore, the return value must be correct -errno in posix semantic 
 */

#define SET_USERFS_FD(fd) fd + g_fd_start
#define GET_USERFS_FD(fd) fd - g_fd_start

#if 0
// Is the directory dp empty except for "." and ".." ?
static int isdirempty(struct inode *dp)
{
	int off;
	struct dirent de;

	for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
		if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
			panic("isdirempty: readi");
		if(de.inum != 0)
			return 0;
	}
	return 1;
}
#endif

int userfs_posix_open(char *path, int flags, uint16_t mode)
{
	struct file *f;
	struct inode *inode =NULL;
	int fd;

	if (flags & O_CREAT) {
		if (flags & O_DIRECTORY)
			panic("O_DIRECTORY cannot be set with O_CREAT\n");
		
		inode = userfs_open_file(path, T_FILE);
		if(inode ==NULL){
			printf("open error\n");
			return -1;
		}

		if (!inode) {
			return -ENOENT;
		}
	} else {
		// opendir API
		if (flags & O_DIRECTORY) {
			// Fall through..
			// it is OK to return fd for directory. glibc allocates 
			// DIR structure and fill it with fd and result from stats. 
			// check: sysdeps/posix/opendir.c
		}

		if ((inode = namei(path)) == NULL) {
			return -ENOENT;
		}
		if (inode->itype == T_DIR) {
			if (!(flags |= (O_RDONLY|O_DIRECTORY))) {
				return -EACCES;
			}
		}
	}
	if(inode->devid < writethdnum){
		inode->devid=0;
	}

	f = userfs_file_alloc();
	if (f == NULL) {
		iunlockput(inode);
		printf("userfs_file_alloc error\n");
		return -ENOMEM;
	}
	
	fd = f->fd;
	inode->fd = f->fd;
	
	pthread_rwlock_wrlock(&f->rwlock);
	
	if (flags & O_DIRECTORY) {
		userfs_debug("directory file inum %d\n", inode->inum);
		f->type = FD_DIR;
	} else {
		f->type = FD_INODE;
	}
	
	f->ip = inode;
	f->readable = !(flags & O_WRONLY);
	f->writable = (flags & O_WRONLY) || (flags & O_RDWR);

	f->off = 0;
	strncpy(f->filename,path,strlen(path));
	f->filename[strlen(path)]='\0';
	

	pthread_rwlock_unlock(&f->rwlock);
	return SET_USERFS_FD(fd);
}

int userfs_posix_access(char *pathname, int mode)
{
	
	struct inode *inode;

	if (mode != F_OK)
		panic("does not support other than F_OK\n");

	inode = namei(pathname);

	if (!inode) {
		return -ENOENT;
	}

	iput(inode);
	return 0;
}

int userfs_posix_creat(char *path, uint16_t mode)
{
	return userfs_posix_open(path, O_CREAT|O_RDWR, mode);
}

int userfs_posix_read(int fd, uint8_t *buf, int count)
{
	
	fd=GET_USERFS_FD(fd);
	struct file *f = &g_fd_table.open_files[fd];

	pthread_rwlock_rdlock(&f->rwlock);
	userfs_assert(f);
	
	if (f->ref == 0) {
		panic("file descriptor is wrong\n");
		return -EBADF;
	}
	
	
	//struct inode * inode = f->ip;
	int ret = userfs_file_read(f, buf, count);
	
	pthread_rwlock_unlock(&f->rwlock);
	if(ret!=count)
		printf("read not success, read ret %d, read size %d\n",ret,count);
	return count;
}

int userfs_posix_pread64(int fd, uint8_t *buf, int count, loff_t off)
{
	int ret = 0;
	struct file *f;
	fd =GET_USERFS_FD(fd);
	f = &g_fd_table.open_files[fd];

	pthread_rwlock_rdlock(&f->rwlock);

	userfs_assert(f);

	if (f->ref == 0) {
		panic("file descriptor is wrong\n");
		return -EBADF;
	}

	ret = userfs_file_read_offset(f, buf, count, off);

	pthread_rwlock_unlock(&f->rwlock);

	return ret;
}

uint32_t allocatefile(){
	struct list_head *pos, *node;
	struct Fileno *fino;
	pthread_mutex_lock(fileno_lock);
	list_for_each_safe(pos,node,&(filenos.list))
	{
		fino = list_entry(pos, struct Fileno, list);
		if(fino->fno>0){
			break;
		}
	}
	list_del(pos);
	
	pthread_mutex_unlock(fileno_lock);
	
	return fino->fno;
}



int userfs_posix_write(int fd, uint8_t *buf, size_t count)
{
	int ret;
	struct file *f;
	fd =GET_USERFS_FD(fd);
	f = &g_fd_table.open_files[fd];
	ret= pthread_rwlock_wrlock(&f->rwlock);

	if(ret < 0){
		panic("lock error\n");
	}
	userfs_assert(f);
	struct inode * inode = f->ip;

	
	if (f->ref == 0) {
		panic("file descriptor is wrong\n");
		return -EBADF;
	}

	ret = userfs_file_write(f, buf, count);
	

	pthread_rwlock_unlock(&f->rwlock);
	
	return ret;
}

int userfs_posix_lseek(int fd, int64_t offset, int origin)
{
	struct file *f;
	int ret = 0;

	fd =GET_USERFS_FD(fd);
	f = &g_fd_table.open_files[fd];
	
	pthread_rwlock_wrlock(&f->rwlock);

	if (f->ref == 0) {
		return -EBADF;
	}

	userfs_assert(f);
	switch(origin) {
		case SEEK_SET:
			f->off = offset;
			break;
		case SEEK_CUR:
			f->off += offset;
			break;
		case SEEK_END:
			f->ip->size += offset;
			f->off = f->ip->size;
			break;
		default:
			ret = -EINVAL;
			break;
	}

	//unlock file
	pthread_rwlock_unlock(&f->rwlock);
	return f->off;
}

int userfs_posix_close(int fd)
{
	struct file *f;
	fd =GET_USERFS_FD(fd);
	f = &g_fd_table.open_files[fd];

	if (!f) {
		return -EBADF;
	}

	return userfs_file_close(f);
}

int userfs_posix_mkdir(char *path, mode_t mode)
{
	int ret = 0;
	struct inode *inode;

	
	// return inode with holding ilock.
	inode = userfs_open_file(path, T_DIR);
	if (!inode) {
		return -ENOENT;
	}

exit_mkdir:
	return ret;
}

int userfs_posix_rmdir(char *path)
{
	int ret = 0;
	struct inode *dir_inode;


	dir_inode = namei(path);

	if (!dir_inode) {
		return -ENOENT;
	}

	if (dir_inode->size > 0) {
		return -EINVAL;
	}

	userfs_debug("%s\n", path);
	dir_remove_entry(dir_inode, path, dir_inode->inum);
	iunlockput(dir_inode);

exit_rmdir:
	return ret;
}

int userfs_posix_stat(const char *filename, struct stat *stat_buf)
{
	struct inode *inode;

	inode = namei((char *)filename);

	if (!inode) {
		return -ENOENT;
	}

	stati(inode, stat_buf);

	return 0;
}

int userfs_posix_fstat(int fd, struct stat *stat_buf)
{
	struct file *f;
	fd =GET_USERFS_FD(fd);
	f = &g_fd_table.open_files[fd];

	if (f->ref == 0) 
		return -ENOENT;

	userfs_assert(f->ip);

	stati(f->ip, stat_buf);

	return 0;
}

#define ALLOC_IO_SIZE (64UL << 10)
int userfs_posix_fallocate(int fd, offset_t offset, offset_t len)
{
	struct file *f;
	int ret = 0;
	size_t i, io_size;
	char falloc_buf[ALLOC_IO_SIZE];
	fd =GET_USERFS_FD(fd);
	f = &g_fd_table.open_files[fd];

	if (f->ref == 0)
		return -EBADF;

	memset(falloc_buf, 0, ALLOC_IO_SIZE);

	userfs_assert(f->ip);

	if (offset > f->ip->size)
		panic("does not support sparse file\n");

	f->off = offset;

	for (i = 0; i < len; i += ALLOC_IO_SIZE) {
		io_size = _min(len - i, ALLOC_IO_SIZE);

		f->off += io_size;

		ret = userfs_file_write(f, (uint8_t *)falloc_buf, io_size);

		if (ret < 0) {
			panic("fail to do fallocate\n");
			return ret;
		}
	}

	return 0;
}

int userfs_posix_unlink(const char *filename)
{
	int ret = 0;
	char name[DIRSIZ];
	struct inode *inode;
	struct inode *dir_inode;

	/* TODO: handle struct file deletion
	 * e.g., unlink without calling close */

	dir_inode = nameiparent((char *)filename, name);
	if (!dir_inode)
		return -ENOENT;
	
	inode = namei((char *)filename);
	if(inode)
		inode->i_ref++;
	
	
	if(!inode)
		inode = dir_lookup(dir_inode, name, NULL);
	if (!inode)  
		return -ENOENT;

	ret = dir_remove_entry(dir_inode, name, inode->inum);
	if (ret < 0) {
		return ret;
	}

	dlookup_del(filename);

	iput(dir_inode);
	iput(inode);
	ret = idealloc(inode);
	
	return ret;
}

int userfs_posix_truncate(const char *filename, offset_t length)
{
	struct inode *inode;
	inode = namei((char *)filename);

	if (!inode) {
		return -ENOENT;
	}

	
	itrunc(inode, length);

	struct timeval curtime;
	struct fuckfs_extent *fsextent=(struct fuckfs_extent *)malloc(sizeof(struct fuckfs_extent));
	fsextent->entry_type=FILE_WRITE;
	fsextent->ino=inode->inum;
	fsextent->isappend =0;
	fsextent->size = length;
	gettimeofday(&curtime,NULL);
	fsextent->mtime =curtime.tv_sec;
	append_entry_log(fsextent,0);

	iput(inode);
	return 0;
}

int userfs_posix_ftruncate(int fd, offset_t length)
{
	struct file *f;
	int ret = 0;
	fd =GET_USERFS_FD(fd);
	f = &g_fd_table.open_files[fd];

	if (f->ref == 0) {
		return -EBADF;
	}


	itrunc(f->ip, length);

	struct timeval curtime;
	struct fuckfs_extent *fsextent=(struct fuckfs_extent *)malloc(sizeof(struct fuckfs_extent));
	fsextent->entry_type=FILE_WRITE;
	fsextent->ino=f->ip->inum;
	fsextent->isappend =0;
	fsextent->size = length;
	gettimeofday(&curtime,NULL);
	fsextent->mtime =curtime.tv_sec;
	append_entry_log(fsextent,0);
	return 0;
}

int userfs_posix_rename(char *oldpath, char *newpath)
{
	int ret = 0;
	struct inode *old_dir_inode, *new_dir_inode;
	char old_file_name[DIRSIZ], new_file_name[DIRSIZ];

	old_dir_inode = nameiparent((char *)oldpath, old_file_name);
	new_dir_inode = nameiparent((char *)newpath, new_file_name);

	userfs_assert(old_dir_inode);
	userfs_assert(new_dir_inode);

	if (old_dir_inode != new_dir_inode)
		panic("Only support rename in a same directory\n");

	

	userfs_assert(strlen(old_file_name) <= DIRSIZ);
	userfs_assert(strlen(new_file_name) <= DIRSIZ);

	dlookup_del(newpath); 

	ret = dir_change_entry(old_dir_inode, old_file_name, new_file_name);
	if (ret < 0) {

		iput(old_dir_inode);
		iput(new_dir_inode);

		dlookup_del(oldpath);
		return ret;
	}

	userfs_debug("rename %s to %s\n", old_file_name, new_file_name);

	dlookup_alloc_add(old_dir_inode,new_file_name);
	dlookup_del( oldpath);

	iput(old_dir_inode);
	iput(new_dir_inode);

	return 0;
}

size_t userfs_posix_getdents(int fd, struct linux_dirent *buf, 
		size_t nbytes, offset_t off)
{
	struct file *f;
	int bytes;
	fd =GET_USERFS_FD(fd);
	f = &g_fd_table.open_files[fd];

	if (f->ref == 0) {
		return -EBADF;
	}

	if (f->type != FD_DIR) 
		return -EBADF;

	/* glibc compute bytes with struct linux_dirent
	 * but ip->size is is computed by struct dirent, 
	 * which is much small size than struct linux_dirent
	if (nbytes < f->ip->size) 
		return -EINVAL;
	*/
	
	if (f->off >= f->ip->size)
		return 0;

	bytes = dir_get_entry(f->ip, buf, f->off);
	f->off += bytes;

	return sizeof(struct linux_dirent);
}

int userfs_posix_fcntl(int fd, int cmd, void *arg)
{
	struct file *f;
	int ret = 0;
	fd =GET_USERFS_FD(fd);
	f = &g_fd_table.open_files[fd];

	if (f->ref == 0) {
		return -EBADF;
	}

	if (cmd != F_SETLK) {
		userfs_debug("%s: cmd %d\n", __func__, cmd);
		//panic("Only support F_SETLK\n");
	}
	return 0;
}

#ifdef __cplusplus
}
#endif
