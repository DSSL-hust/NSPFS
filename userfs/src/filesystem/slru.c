...
int slru_upsert(struct inode *inode, struct list_head *lru_head, lru_key_t k, lru_val_t v) 
{
	...
	if (!node) {
		//node = (lru_node_t *)userfs_alloc(sizeof(lru_node_t));
		node = (slru_node_t *)userfs_alloc_shared(sizeof(slru_node_t));
		// if forgot to this memset, UThash does not work.
		memset(node, 0, sizeof(slru_node_t));

		userfs_debug("add a new key: dev %u, block%lx\n", k.dev, k.block);
	...
	if (inode) {
		...
		list_add_tail(&node->per_inode_list, &inode->i_slru_head);
	}
	*/

	// Add to head of lru_list.
	list_del_init(&node->list);
	list_add_tail(&node->list, lru_head);
