...
int userfs_condvar_wait(struct userfs_condvar *cv, struct userfs_mutex *mu)
{
	int seq = cv->seq;

	if (cv->m != &mu->lock) {
		userfs_assert(cv->m == NULL);

		cmpxchg(cv->m, NULL, &mu->lock);

		userfs_assert(cv->m == &mu->lock);
	}
	
	userfs_mutex_unlock(mu);
...
int userfs_condvar_wait(struct userfs_condvar *c, userfs_mutex_t *m)
{
	int seq = c->seq;
